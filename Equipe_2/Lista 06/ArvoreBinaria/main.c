...
int main() {
    ...
    PNoArvBin busca = NULL;

    // Inserção aleatória de elementos na árvore
    srand(time(NULL));
    for (int i = 0; i < 10; i++) {
        key = rand() % tam;
        arvore = insereNoArvBin(arvore, acervo[key]);
    }
    ...
    key = 3;
    busca = buscaNaArvAVL(arvore, key);
    if (busca) {
        printf("Jogo encontrado: (%d, %s)\n", busca->jogo->appId, busca->jogo->title);
    } else {
    ...
    key = 5;
    arvore = removeDaArvAVL(arvore, key);
    printf("Elemento removido com sucesso!\n");
    printf("\n");

    // Impressão da árvore após a remoção
    printf("Impressão após remoção:\n");
     printArv(arvore, 0);
    printf("\n");
    printf("------------------\n");
    printArv(arvore, 1);
...
Jogos *carregaDados(char *fileName, int *tam)
{
    ...
    while (!feof(fp) && i < *tam)
    {
        ...
        if (ok)
        {
            ...
            int pos = 0;
            while (campo[pos] != 15 && pos < (int)strlen(campo))
                pos++;
            campo[pos] = '\0';
