...
PNoArvBin removeDaArvAVL(PNoArvBin arv, Jogos jogo) {
    if (!arv) {
        ...
        if ( arv->jogo->appId == jogo.appId ) {
            if(!arv->esq && !arv->dir) {
                ...
                if (arv->esq && arv->dir) {
                    ...
                    while (aux->dir){
                        ...
                        arv->jogo = aux->jogo;
                        aux->jogo = jogo.appId;
                        arv -> esq = removeNoArv(arv->esq, jogo);
                        return arv; }
                } else {
                    PNoArvBin aux;
                    if (arv -> esq){
            ...
            if (jogo.appId < arv->jogo->appId){ 
                arv -> esq = removeNoArv(arv -> esq, jogo);
            }
            else{
                arv -> dir = removeNoArv(arv -> dir, jogo);
            }
        }
        arv->altura = maior(alturaNoArv(arv -> esq), alturaNoArv(arv -> dir)) + 1;
        arv = balanceia(arv);
        return arv;
    }
...
void printArv(PNoArvBin arv)
{
    ...
    {
        printArv(arv->esq);
        printf("(%s, %s)  ", arv->jogo->appId, arv->jogo->title, arv->altura);
        printArv(arv->dir);
    }      
