...
int main() {
    ...
    //Preenchimento ordenado - SubColeção
    ...
    while(tamanhoColecao < capacidadeColecao) { // enquanto não estiver cheia
        ...
        if ( incOrdRegistro(acervo[posicaoAleatoria], subColecao, &tamanhoColecao )) conseguiuAdicionar++;
        tentativa++;
    }
    ...
    while(tamanhoColecao > conseguiuBuscar) {
        ...
        if ( buscaBinPorId(acervo[posicaoAleatoria].appId, subColecao, tamanhoColecao) >= 0 ) conseguiuBuscar++;
        tentativa++;
    ...
    while(tamanhoColecao > 0) { 
        int posicaoAleatoria = rand()%tamanhoAcervo;
        
        if ( remOrdRegistro(acervo[posicaoAleatoria].appId, subColecao, &tamanhoColecao) ) conseguiuRemover++;
        tentativa++;
    }
