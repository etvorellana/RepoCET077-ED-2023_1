...
//////////////////////////////////////////////////////
///////////////////PROTOTIPOS////////////////////////
t_dados *carregaDados(const char *arquivo, int *tam);
void limpaArquivo(t_dados *netflix, int tam);
int procurar_id_estoque(t_dados *netflix, int n_registros, char *id_estoque_procurado);
...
int main() {
    ...
    netflix = carregaDados("netflix.csv", &tam);
            ...
            // busca por um idEstoque especifico
            char idBuscado[16];

            printf("digite o id de estoque do produto: ");
            scanf("%s", idBuscado);
            int index = procurar_id_estoque(netflix, &tam, idBuscado); //indice recebe por Parametro: lista do arquivo, qtde de registros, idDesejado
...
void remover_elemento_do_subArquivo(char *subArquivo, char *id) {
    ...
    FILE *temp = fopen("temp.txt", "w"); //copiar todas as linhas que não contêm o ID especificado para o arquivo temporário temp. 
    ...
    rename("temp.txt", subArquivo); //renomear o arquivo temporário temp.txt para o nome do arquivo subArquivo
}